...
@section('content')
    <div class="flex gap-5 flex-col">
        <div class="w-full flex gap-5">
            <form action="{{ route('admin.update.course', $course->id) }}" method="post" class="flex gap-5 w-full" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="w-3/4 bg-white rounded-xl px-4 pt-4 pb-3 flex flex-col gap-3 justify-between">
                    ...
                    <button data-modal-target="popup-modal-{{ $course->id }}" data-modal-toggle="popup-modal-{{ $course->id }}" class="bg-red-500 hover:bg-red-800 text-white font-medium px-5 py-2 rounded-lg" type="button">
                       Удалить курс
                    </button>
                </div>
            </form>
