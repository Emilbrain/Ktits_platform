...
@section('content')
    <div class="flex gap-5 flex-col">
        <div class="bg-white rounded-xl px-5 pt-5 pb-4 flex flex-col">
            ...
            <div class="flex"></div>
            @if(auth()->user()->logo)
                <img src="{{ asset('storage/'  . auth()->user()->logo) }}" alt="" class="w-16 flex justify-center mb-4">
            @else
                <img src="{{ asset('images/user.png') }}" alt="" class="w-16 flex justify-center mb-4">
            @endif

            <form action="{{route('admin.setting.update.avatar')}}" method="post" enctype="multipart/form-data">
                @csrf
                <input
                        class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400"
                        aria-describedby="file_input_help" name="logo" id="file_input" type="file">
...
{{--            <form action="{{ route('admin.setting.update.telegram') }}" method="post">--}}
{{--                @csrf--}}
{{--                <label for="telegram_username"--}}
{{--                       class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">@if(isset(auth()->user()->telegram_username))--}}
{{--                        Тукущий ник: {{ auth()->user()->telegram_username }}--}}
{{--                    @else--}}
{{--                        Ник--}}
{{--                    @endif</label>--}}
...
{{--                <p class="mt-1 text-sm text-gray-500 dark:text-gray-300" id="file_input_help">Указывайте корректный ник--}}
{{--                    (без @)</p>--}}
{{--                <input type="submit"--}}
{{--                       class="bg-gray-900 mt-3 cursor-pointer text-white hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800"--}}
{{--                       value="Сменить">--}}
...
{{--        </div>--}}
    ...
    <div class="bg-white rounded-xl px-5 pt-5 pb-4 flex flex-col justify-between relative mt-10">
        <h2 class="text-xl mb-4">Сменить пароль</h2>
        <form action="{{ route('admin.setting.update.password') }}" method="post">
            @csrf
            <label for="current_password" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Текущий
                пароль</label>
            ...
            <label for="new_password" class="block mt-3 mb-2 text-sm font-medium text-gray-900 dark:text-white">Новый
                пароль</label>
            ...
            <label for="new_password_confirmation"
                   class="block mt-3 mb-2 text-sm font-medium text-gray-900 dark:text-white">Подтверждение
                пароля</label>
            ...
            <input type="submit"
                   class="bg-gray-900 mt-4 cursor-pointer text-white hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800"
    ...
    {{--                    <p class="text-sm font-normal text-gray-500 dark:text-gray-400">Для старта получения уведомлений нажмите у бота кнопку "start".</p>--}}
